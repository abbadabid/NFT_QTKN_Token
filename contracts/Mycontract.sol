...
contract Mycontract is Initializable, OwnableUpgradeable {
    ...
    mapping(string=>uint256) name_to_nft_id;
    ...
    function buyNft(uint256 _course_id)public{
        ...
        uint256 nft_id = mynft.safeMint(msg.sender);
        //mynft.safeMint(msg.sender);
        console.log(nft_id);
        name_to_nft_id[id_to_course[_course_id].course_name]= nft_id;
        courses[_course_id].owner = msg.sender; 
    ...
    function showNftId(string memory course_name)public view returns(uint256){
        return name_to_nft_id[course_name];
    }
